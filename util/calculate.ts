class Calculate {
  ...
  static bezier(
    ...
    const clamp = (a: any, min: any, max: any) =>
  ...
  static hermite(controlPoints: string | any[], lowBound: any, highBound: any) {
    ...
    const add = (a: any[], b: any[], c: any[], d: any[]) => [
      a[0] + b[0] + c[0] + d[0],
      a[1] + b[1] + c[1] + d[1],
    ];
  ...
  // to using convolution tric to change image using matrix
  //   Parameters
  // inputData (Type: Buffer): The input image data represented as a Buffer. This is the image on which convolution will be applied.

  // kernel (Type: number[][]): The convolution kernel, a 2D array of numbers. The kernel defines the convolution filter that will be applied to the input image. It is typically a square matrix with an odd number of rows and columns.

  // width (Type: number): The width of the input image in pixels.

  // height (Type: number): The height of the input image in pixels.

  // Return Value
  // The function returns a new Buffer containing the convoluted image data.
  // Convolution Process
  // The convolution function applies convolution to the input image using the provided kernel. Here's an overview of the convolution process:

  // It creates a new Buffer newData to store the result, initialized with the same data as the input image.

  // It calculates the dimensions of the kernel and determines the starting and ending indices for row and column loops.

  // It iterates over each pixel in the input image, and for each pixel:

  // a. It initializes variables rSum, gSum, and bSum to store the weighted sums for the red, green, and blue color channels, respectively.

  // b. It performs a nested loop to iterate over the elements of the kernel, centered around the current pixel.

  // c. For each element in the kernel, it calculates a weighted sum of the corresponding neighboring pixel values from the input image.

  // d. It clamps the resulting sums to the range [0, 255] to ensure they stay within valid color channel values.

  // e. It updates the corresponding pixel values in the newData Buffer with the clamped sums.

  // Finally, it returns the newData Buffer containing the convoluted image.

  // Note
  // The convolution operation can be used for various image processing tasks by providing different kernels. The specific operation (e.g., blurring, sharpening) depends on the values within the kernel.
  static convolution(
    inputData: Buffer,
    kernel: number[][],
    width: number,
  ...
  // to get index pixel
  //   Parameters
  // x (Type: number): The x-coordinate of the pixel in the 2D image grid.

  // y (Type: number): The y-coordinate of the pixel in the 2D image grid.

  // width (Type: number): The width of the image grid in pixels. It represents the number of pixels in each row of the image.

  // height (Type: number): The height of the image grid in pixels. It represents the number of rows in the image.
  static getPixelIndex(
    x: number,
    y: number,
    width: number,
    height: number,
